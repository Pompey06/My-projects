			...
			function (module, exports, __webpack_require__) {
				...
				(Inputmask.prototype = {
					...
					defaults: {
						...
						ignorables: [
							8, 9, 19, 27, 33, 34, 35, 36, 37, 38, 39, 40, 45,
									...
									if (
										...
										for (var nmsp in eventRegistry[ev])
											for (
												i = 0;
												i <
												eventRegistry[ev][nmsp].length;
												i++
											)
												eventRegistry[ev][nmsp][
													i
								...
								for (name in options)
									(src = target[name]),
										...
										target !== copy &&
											(deep &&
											copy &&
											(DependencyLib.isPlainObject(
												copy
											) ||
												(copyIsArray =
													DependencyLib.isArray(
														copy
													)))
												? ((clone = copyIsArray
														? ((copyIsArray = !1),
			...
			function (module, exports, __webpack_require__) {
				...
				module.exports = function maskScope(actionObj) {
					var inputmask = this,
						...
						isRTL =
							inputmask.isRTL ||
							(inputmask.isRTL = opts.numericInput);
					...
					function getTests(pos, ndxIntlzr, tstPs) {
						...
						function resolveTestFromToken(
							...
							for (
								...
								if (
									!0 !== maskToken.matches[tndx].isQuantifier
								) {
						...
						switch (opts.casing || test.casing) {
							...
							case 'title':
								...
								elem =
									...
									(posBefore &&
										posBefore.input ===
											String.fromCharCode(keyCode.SPACE))
										? elem.toUpperCase()
										: elem.toLowerCase();
								break;
							default:
							...
							switch (opts.positionCaretOnClick) {
								...
								default:
									...
									if (clickPosition < lastPosition)
										...
										selectedCaret.end =
											selectedCaret.begin = lastPosition;
									}
							}
						...
						function isTemplateMatch(ndx, charCodes) {
							for (
								var targetTemplate = getMaskTemplate(!0, 0)
										...
										.replace(/'/g, ''),
									charCodeNdx =
										targetTemplate.indexOf(charCodes);
								0 < charCodeNdx &&
					...
					if (void 0 !== actionObj)
						switch (actionObj.action) {
							case 'isComplete':
								return (
									(el = actionObj.el), isComplete(getBuffer())
								);
							case 'unmaskedvalue':
								return (
									(void 0 !== el &&
					...
					void 0 === opts.parseMinMaxOptions &&
						(null !== opts.min &&
							((opts.min = opts.min
								...
								.replace(
									new RegExp(
										escapeRegex(opts.groupSeparator),
										'g'
									),
									''
								)),
					...
					!0 === opts.numericInput && void 0 === opts.__financeInput
						? ((decimalDef = '1'),
						  ...
						  '' !== opts.radixPoint &&
								...
								(opts.definitions[radixPointDef].generated =
									!0)))
...
const fullAngle = 2 * Math.PI - 2 * startAngle;
...
// Цвета
// 🎨 Цвета (можно менять)
const borderColor = '#444'; // Цвет границы
const fillColorStart = '#fff'; // Начальный цвет заливки
const fillColorEnd = '#fff'; // Конечный цвет заливки
const circleColor = '#007bff'; // Цвет точки
...
function drawSpeed(start, end, cur) {
	drawArc(start, start + fullAngle, borderColor); // Граница
	drawArc(start, cur, grad); // Заливка
	drawCircle(cur); // Точка
}
...
// Выносим в глобальный объект window
window.setScore = function (percent) {
	percent = Math.max(0, Math.min(100, percent)); // Ограничиваем 0-100%
	endAngle = startAngle + fullAngle * (percent / 100);
	curAngle = startAngle;
	animateProgress();
};

let checkInput = document.querySelector('.check__input');
let inputt = document.querySelector('.check__input input');
let blurWp = document.querySelector('.blur-wp');

inputt.addEventListener('input', function () {
	console.log(inputt.value.length);
	if (inputt.value.length > 0) {
		checkInput.classList.add('_active');
	} else {
		checkInput.classList.remove('_active');
	}
});
...
document.addEventListener('click', function (e) {
	e = e.target;
	if (e.classList.contains('check__address')) {
		blurWp.classList.remove('_active');
		checkInput.classList.remove('_active');
		inputt.value = '';
	}
});
